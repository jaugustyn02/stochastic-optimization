...
get_means <- function(num_runs, num_points, num_dims, obj_fn){
  ...
  png(file = paste("mshist", num_dims, ".png", sep = ""))
  hist(ms, plot=TRUE, xlab = "Weight",col = "yellow",border = "blue")
  dev.off()
  ms_mean <- mean(ms)
  ms_sd <- sd(ms)
  ...
  png(file = paste("prshist", num_dims, ".png", sep = ""))
  hist(prs, plot=TRUE, xlab = "Weight",col = "yellow",border = "blue")
  dev.off()
  cat(length(prs))
  prs_mean <- mean(prs)
  prs_sd <- sd(prs)
  prs_confidence <- c(prs_mean - qnorm(.975, sd=prs_sd)*prs_sd/sqrt(prs_num_runs), prs_mean + qnorm(.975, sd=prs_sd)*prs_sd/sqrt(prs_num_runs))
